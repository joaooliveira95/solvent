...
class DuckDuckGoSearchAPIWrapper(BaseModel):
    ...
    def _ddgs_text(self, query: str, max_results: Optional[int] = None) -> List[Dict[str, str]]:
        ...
        try:
            ...
            except (DuckDuckGoSearchException, RatelimitException, TimeoutException) as e:
                with DDGS(proxy=cfg.SQUID_PROXY) as ddgs:
                    ddgs_gen = ddgs.text(
                        query,
                        region=self.region,
                        safesearch=self.safesearch,
            ...
            logging.exception("DuckDuckGo search exception")
            ddgs_gen = []
        if ddgs_gen:
