"""
Modified ChatInterface for voice support
"""
...
from gradio import HTML, Checkbox
from elevenlabs.client import ElevenLabs
from elevenlabs import Voice, VoiceSettings

from solventgpt.voiceutils import strip_markdown
from solventgpt import config as cfg


@document()
class VoiceChatInterface(Blocks):
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.elevenlabs_client = ElevenLabs(api_key=cfg.ELEVENLABS_API_KEY)
        ...
        with self:
            ...
            self.voice_active = Checkbox(value=False, label="Activate Voice Output")
            ...
            with Group():
                with Row():
                    if textbox:
                        ...
                        self.audio_playback = HTML(visible=False)
                        # self.audio_record = Audio(sources=["microphone"], type="filepath", scale=4)
    ...
    def _setup_events(self) -> None:
        ...
        submit_event = (
            ...
            .then(self._generate_audio, [self.chatbot_state, self.voice_active], [self.audio_playback])
        )
    ...
    async def _generate_audio(self, history, voice_active):
        audio_player = ""
        if voice_active:
            voice = Voice(
                voice_id="nPczCjzI2devNBz1zQrb",
                settings=VoiceSettings(stability=0.31, similarity_boost=0.6, style=0.4, use_speaker_boost=True),
            )

            last_reply = strip_markdown(history[-1][1])
            audio = self.elevenlabs_client.generate(text=last_reply, voice=voice)
